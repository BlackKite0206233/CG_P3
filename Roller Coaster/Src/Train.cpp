...
void CTrain::Draw(bool doingShadows, bool isSelected, Light& light, QVector3D& eyePos, SSAOFrameBuffer* ssaoFrameBuffer, QVector4D& clipPlane) {
	PathData pd = track->GetPath(p0, p1, p2, p3);
	SetNewPos(pd);
	...
	this->model->render(color, ProjectionMatrex, ViewMatrex, modelMatrix, light, eyePos, ssaoFrameBuffer, clipPlane, 25);
	...
	for (int i = 0; i < car.size(); i++) {
		...
		car[i].Draw(doingShadows, isSelected, light, eyePos, ssaoFrameBuffer, clipPlane);
	}
}

void CTrain::DrawGeometry(QOpenGLShaderProgram* shader) {
	this->model->DrawGeometry(modelMatrix, QOpenGLShaderProgram* shader, 25);
	for (int i = 0; i < car.size(); i++) {
